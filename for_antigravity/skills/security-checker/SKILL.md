...
description: API 키, 토큰 등 민감한 정보의 노출을 방지하기 위한 보안 점검을 수행합니다.
---
# Security Checker
...
Antigravity는 GitHub에 프로젝트를 올리기 전, API Key, token, password, `credentials.json`, `auth.json`, `.env` 파일, 개인정보 포함 데이터가 코드나 문서에 노출되지 않았는지 확인해야 합니다.
...
Antigravity는 보안 점검 결과를 아래 형식으로 제공합니다.
...
보안 위험을 발견한 경우 Antigravity는 실제 값을 출력하지 않고, 파일 위치와 조치 방법만 안내합니다.
