...
```powershell
Test-Path $HOME\.antigravity\AGENTS.md
Get-Content -Encoding UTF8 $HOME\.antigravity\AGENTS.md | Select-Object -First 5
```
...
```powershell
Test-Path $HOME\.antigravity\skills\requirements-definition\SKILL.md
```

### 6. config 설치 확인

```powershell
dir $HOME\.antigravity\config
```
...
```powershell
Get-Content -Encoding UTF8 $HOME\.antigravity\config\lean-skills.txt
```
...
| `~/.antigravity`에 파일이 없음 | 설치 중 오류 메시지를 확인하고 스크립트를 다시 실행합니다. |
...
```bash
test -f ~/.antigravity/AGENTS.md && echo "AGENTS.md installed"
head -n 5 ~/.antigravity/AGENTS.md
```
...
```bash
test -f ~/.antigravity/skills/requirements-definition/SKILL.md && echo "requirements-definition installed"
```

### 6. config 설치 확인

```bash
ls ~/.antigravity/config
```
...
```bash
cat ~/.antigravity/config/lean-skills.txt
```
...
| `~/.antigravity`에 파일이 없음 | 설치 중 오류 메시지를 확인하고 스크립트를 다시 실행합니다. |
| 기존 `AGENTS.md`가 사라진 것 같음 | `ls ~/.antigravity/AGENTS.md.backup.*`로 백업 파일을 확인합니다. |
| Skill 폴더가 일부 없음 | 저장소가 최신인지 확인하고 다시 clone합니다. |
| Antigravity가 Kit 흐름을 따르지 않음 | `~/.antigravity/AGENTS.md`와 `~/.antigravity/skills`가 설치되었는지 다시 확인합니다. |
