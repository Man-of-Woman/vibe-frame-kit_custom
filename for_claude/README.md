...
`vibe-frame-kit`은 재직자 AI 서비스 개발 과정 수강생들이 Claude를 활용해 프로젝트를 진행할 수 있도록 돕는 공통 Vibe Frame Kit입니다.
...
이 Kit는 Claude와 함께 프로젝트를 진행할 때 다음과 같은 공통 기준을 제공합니다.
...
- Claude를 활용해 AI 서비스 프로젝트를 진행하려는 학습자
...
| 기능 구현 | Claude와 함께 작은 작업 단위로 코드를 작성합니다. |
...
| `CLAUDE.md` | Claude가 프로젝트에서 따를 공통 작업 규칙을 정의합니다. |
...
| `config/` | 기본 Skill 목록과 Claude 설정 샘플을 보관합니다. |
...
이 프레임워크 배포판은 **Claude** 전용이며, 사용자 홈 디렉토리의 `~/.claude`에 설치되어 전역 지시사항 규칙(`CLAUDE.md`)으로 연동됩니다.
...
- Claude가 생성한 코드는 반드시 직접 실행하고 확인해야 합니다.
...
- Claude의 제안이 항상 최선은 아니므로 수강생이 직접 판단하고 검토해야 합니다.
...
- Claude에 실제 비밀번호나 개인 인증 정보를 입력하지 않습니다.
