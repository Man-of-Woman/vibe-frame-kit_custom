...
Antigravity는 요구사항 정의서, 기능 분해표, MVP 계획서, 프로젝트 구조를 바탕으로 API 명세를 작성하고, Request/Response 구조, Pydantic schema, router, service, 예외 처리, Swagger 테스트 방법을 함께 정리해야 합니다.
...
Antigravity는 API 설계와 구현 결과를 다음 형식으로 제공합니다.
...
코드 수정이 포함된 경우 Antigravity는 어떤 파일을 왜 수정했는지 함께 설명해야 합니다.
