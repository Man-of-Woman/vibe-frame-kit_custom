...
```powershell
Test-Path {{INSTALL_PATH}}\{{RULES_FILE}}
Get-Content -Encoding UTF8 {{INSTALL_PATH}}\{{RULES_FILE}} | Select-Object -First 5
```

확인 기준:

- `True`가 출력됩니다.
- `{{RULES_FILE}}` 상단 내용이 한국어로 표시됩니다.

기존 `{{RULES_FILE}}`가 있었다면 아래와 같은 백업 파일도 생성될 수 있습니다.

```text
{{RULES_FILE}}.backup.yyyymmdd-hhmmss
```
...
```powershell
Test-Path {{INSTALL_PATH}}\skills\requirements-definition\SKILL.md
```

### 6. config 설치 확인

```powershell
dir {{INSTALL_PATH}}\config
```
...
```powershell
Get-Content -Encoding UTF8 {{INSTALL_PATH}}\config\lean-skills.txt
```

### 7. {{AGENT_NAME}} App 또는 CLI에서 확인할 프롬프트

새 프로젝트 폴더로 이동한 뒤 {{AGENT_NAME}}를 실행합니다.
...
{{AGENT_NAME}} App을 사용하는 경우에는 `test-ai-service` 폴더를 열고 아래 프롬프트를 입력합니다.
...
```bash
test -f {{INSTALL_PATH}}/{{RULES_FILE}} && echo "{{RULES_FILE}} installed"
head -n 5 {{INSTALL_PATH}}/{{RULES_FILE}}
```

확인 기준:

- `{{RULES_FILE}} installed`가 출력됩니다.
- `{{RULES_FILE}}` 상단 내용이 한국어로 표시됩니다.
...
```bash
test -f {{INSTALL_PATH}}/skills/requirements-definition/SKILL.md && echo "requirements-definition installed"
```

### 6. config 설치 확인

```bash
ls {{INSTALL_PATH}}/config
```
...
```bash
cat {{INSTALL_PATH}}/config/lean-skills.txt
```
...
| Skill 폴더가 일부 없음 | 저장소가 최신인지 확인하고 다시 clone합니다. |
| {{AGENT_NAME}}가 Kit 흐름을 따르지 않음 | `{{INSTALL_PATH}}/{{RULES_FILE}}`와 `{{INSTALL_PATH}}/skills`가 설치되었는지 다시 확인합니다. |
